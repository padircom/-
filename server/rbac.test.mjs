...
test("کاتالوگ: نسخه موتور و اندازه‌ها", () => {
  ...
  assert.equal(PERMISSION_CATALOG.length, 59);
...
test("SOD: هفت قاعده تعریف شده و شناسه‌ها یکتاست", () => {
  assert.equal(SOD_RULES.length, 7);
  assert.equal(new Set(SOD_RULES.map((r) => r.id)).size, 7);
});

test("SOD: کاربر دو‌نقشی تخلف تهیه/تأیید صورت‌وضعیت را نشان می‌دهد", () => {
  const v = sodViolations(subjectPermissions(subj("u-over")));
  assert.deepEqual(v.map((x) => x.id), ["SOD-01"]);
