...
test("کاتالوگ قالب‌ها سالم است", () => {
  /* شمار قالب‌ها با افزوده شدن ماژول‌ها رشد می‌کند؛ آنچه باید ثابت بماند
   * یکتایی کد و کامل بودن ساختار هر قالب است، نه عدد مطلق. */
  assert.ok(TEMPLATE_CATALOG.length >= 6, "کاتالوگ نباید از شش قالب پایه کمتر شود");
  assert.equal(new Set(TEMPLATE_CATALOG.map((t) => t.code)).size, TEMPLATE_CATALOG.length, "کد قالب تکراری وجود دارد");
