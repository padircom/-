...
export type DcmaCheck = { id: number; nameFa: string; value: number; pass: boolean };
...
export function dcma14(input: DcmaInput): { checks: DcmaCheck[]; healthScore: number } {
  ...
  const checks: DcmaCheck[] = [
    { id: 1, nameFa: "منطق ناقص", value: pct(missingLogic), pass: pct(missingLogic) <= 5 },
    { id: 2, nameFa: "لگ منفی (Lead)", value: leads, pass: leads === 0 },
    { id: 3, nameFa: "شناوری زیاد", value: pct(highFloat), pass: pct(highFloat) <= 5 },
    { id: 4, nameFa: "مدت زیاد", value: pct(highDuration), pass: pct(highDuration) <= 5 },
    { id: 5, nameFa: "قید سخت", value: pct(hardConstraintCount), pass: pct(hardConstraintCount) <= 10 },
    { id: 6, nameFa: "چگالی روابط", value: Math.round(density * 100) / 100, pass: density >= 0.5 && density <= 2 },
    { id: 7, nameFa: "لگ‌ها", value: rels.length ? (lags / rels.length) * 100 : 0, pass: !rels.length || (lags / rels.length) * 100 <= 5 },
    { id: 8, nameFa: "تاریخ نامعتبر", value: invalidDates, pass: invalidDates === 0 },
    { id: 9, nameFa: "منابع مسیر بحرانی", value: cpNoResource, pass: cpNoResource === 0 },
    { id: 10, nameFa: "کارهای عقب‌افتاده", value: pct(missedTasks), pass: pct(missedTasks) <= 5 },
    { id: 11, nameFa: "CPLI", value: Math.round(cpli * 100) / 100, pass: cpli >= 0.95 },
    { id: 12, nameFa: "BEI", value: Math.round(bei * 100) / 100, pass: bei >= 0.95 },
    { id: 13, nameFa: "وجود مسیر بحرانی", value: hasCriticalPath ? 1 : 0, pass: hasCriticalPath },
    { id: 14, nameFa: "توزیع شناوری", value: pct(maxBucket), pass: pct(maxBucket) <= 40 },
  ];
