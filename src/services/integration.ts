...
export function toNumber(value: unknown): number | null {
  /* جداکنندهٔ اعشار فارسی (U+066B) و هزارگان عربی (U+066C) در normalizeDigits
   * ترجمه نمی‌شوند چون آنجا فقط رقم تبدیل می‌شود؛ اینجا لازم‌اند وگرنه «۱۲٫۵»
   * به NaN می‌رسد و ورودی معتبر کاربر بی‌صدا رد می‌شود. */
...
export const TEMPLATE_CATALOG: TemplateDef[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      f("ChecklistFa", "چک‌لیست", "Checklist", "text", "روغن موتور، فیلتر روغن، فیلتر سوخت، فیلتر هوا", { maxLen: 2000, aliases: ["ChecklistFa", "چک‌لیست", "شرح اقدامات"] }),
    ],
  },
  {
    code: "TPL-SPR",
  ...
  },
  {
    ...
    fields: [
      ...
      f("CommentText", "متن نظر", "Comment", "text", "ضخامت فونداسیون با بار محاسباتی همخوان نیست", { required: true, maxLen: 2000, aliases: ["CommentText", "متن نظر", "نظر", "Comment", "Remark"] }),
      f("ResponseText", "پاسخ طراح", "Designer response", "text", "ضخامت به ۹۰ سانتی‌متر افزایش یافت", { maxLen: 2000, aliases: ["ResponseText", "پاسخ طراح", "پاسخ", "Response", "Reply"] }),
...
export function mapHeaders(t: TemplateDef, headers: string[]): HeaderMap {
  ...
  for (const fd of t.fields) {
    /* عنوان خودِ میدان همیشه یک نام مستعار معتبر است. بدون این، فایلی که از
     * `templateCsv` همین قالب تولید شده در برگشت شناخته نمی‌شد — مثلاً سرستون
     * «زمان تأمین (روز)» با نام مستعار «زمان تأمین» یکی نمی‌شد. */
    lookup.set(normHeader(fd.title.fa), fd);
    lookup.set(normHeader(fd.title.en), fd);
    for (const a of fd.aliases) lookup.set(normHeader(a), fd);
  }
